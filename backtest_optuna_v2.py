...
def run_simulation(top_n=3):
    print(f"\n--- PREPARATION DES DONNEES (SMART OPTUNA V2 + TOP {top_n}) ---")
    ...
    print("Génération des prédictions ML (Walk-Forward avec LightGBM + Optuna V2)...")
        ...
        if current_date.month in [1, 4, 7, 10] and current_date.month != last_train_month:
            ...
            for ticker, df in all_features.items():
                ...
                if len(df_hist) > 252 + 5:
                    df_hist = df_hist.iloc[:-5] # Global purge
            ...
            if X_train_list:
                ...
                # Tri par date crucial pour la TimeSeriesSplit
                sort_idx = np.argsort(X_train.index)
                ...
                def objective(trial):
                    param = {
                        # Espace bride pour la finance
                        'n_estimators': 500, # Beaucoup d'arbres, mais Early Stopping s'en occupera
                        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.05),
                        'max_depth': trial.suggest_int('max_depth', 2, 5), # Arbres tres petits
                    ...
                    # Purged TimeSeriesSplit (gap=5 jours pour l'horizon de 5 jours)
                    ...
                    for train_idx, val_idx in tscv.split(X_sorted):
                        ...
                        # Objectif : Information Coefficient (Spearman)
                        coef, _ = spearmanr(y_v, preds)
                        # S'il n'y a pas de variance (coef nan), on penalise
                        if np.isnan(coef): coef = 0 
                        ic_scores.append(coef)
                        
                    return np.mean(ic_scores)
                
                print(f"[{current_date.date()}] Optuna V2 (Spearman + Purged CV + EarlyStop) - 20 essais...")
                # Maximiser l'Information Coefficient
                ...
                best_params['n_estimators'] = 100 # Entrainement final fixe avec les meilleurs params
                
                trained_model = lgb.LGBMRegressor(**best_params)
                trained_model.fit(X_sorted, y_sorted)
    ...
    print(f"Resultats Top {top_n} avec Smart Optuna V2 :")
...
if __name__ == '__main__':
    print("DEMARRAGE DU SETUP STEFAN JANSEN + SMART OPTUNA V2")
    import matplotlib.pyplot as plt
    strat_optuna = run_simulation(top_n=3)
    
    plt.figure(figsize=(12, 6))
    plt.plot(strat_optuna.dates, strat_optuna.portfolio_values, color='gold', linewidth=2)
    plt.title('Performance du Bot : Smart Optuna V2 (Top 3)', fontsize=14)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Valeur du Portefeuille ($)', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.axhline(10000, color='black', linestyle='-', linewidth=1)
    artifact_path = r'C:\Users\Elrik\.gemini\antigravity\brain\3bd380e5-0c43-4284-9645-00b7bc827801\optuna_v2_equity_curve.png'
    plt.tight_layout()
