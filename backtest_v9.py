...
def prepare_features(df_asset, sector_name, s_bench, s_global, s_vix, s_dxy, s_tnx, mode='v9'):
    df = df_asset.copy()
    ...
    if mode == 'v9':
        if not s_dxy.empty:
            df['DXY'] = s_dxy
            df['DXY'] = df['DXY'].ffill()
            df['DXY_Returns'] = df['DXY'].pct_change(periods=5)
        if not s_tnx.empty:
            df['TNX'] = s_tnx
            df['TNX'] = df['TNX'].ffill()
            df['TNX_Change'] = df['TNX'].diff(periods=5)
...
def get_features_list(sector_name, mode, has_dxy, has_tnx):
    features = ['Dist_SMA_10', 'Dist_SMA_50', 'Returns', 'Returns_5d', 'Returns_20d', 
    ...
    if mode == 'v9':
        if has_dxy:
            features.append('DXY_Returns')
        if has_tnx:
            features.append('TNX_Change')
    return features

def run_simulation(data, mode='v8'):
    label = "V8 PRO" if mode == 'v8' else "V9 PRO (DXY+TNX)"
    print(f"\n--- Simulation {label} ---")
    ...
    EMBARGO_DAYS = 15 if mode == 'v9' else 0  # V9 : embargo de 15 jours
        ...
        if current_date.month in [1, 4, 7, 10] and current_date.month != last_train_month:
            ...
            for sector_name in SECTORS.keys():
                ...
                for ticker, d in features_dict[sector_name].items():
                    df = d['df']
                    # Embargo : on coupe les N derniers jours avant la date
                    cutoff = current_date - pd.Timedelta(days=EMBARGO_DAYS)
                    df_hist = df[(df.index < cutoff)].copy()
...
if __name__ == '__main__':
    ...
    # Simulation V8 (SANS DXY/TNX/Embargo)
    eq_v8, cap_v8, ret_v8, dd_v8 = run_simulation(data, mode='v8')
    
    # Simulation V9 (AVEC DXY/TNX/Embargo)
    eq_v9, cap_v9, ret_v9, dd_v9 = run_simulation(data, mode='v9')
    ...
    print("\n================================================")
    print("    COMPARAISON V8 vs V9 PRO (8 ANS)    ")
    print("================================================")
    print(f"S&P 500      : {spy_ret*100:+.2f}%")
    print(f"V8 PRO       : {ret_v8*100:+.2f}% (Drawdown: -{dd_v8*100:.2f}%)")
    print(f"V9 PRO       : {ret_v9*100:+.2f}% (Drawdown: -{dd_v9*100:.2f}%)")
    delta_ret = ret_v9 - ret_v8
    delta_dd = dd_v8 - dd_v9
    print(f"Amelioration : {delta_ret*100:+.2f}% de rendement, {delta_dd*100:+.2f}% de drawdown")
    print("================================================")
    
    # Graphique
    plt.figure(figsize=(14, 8))
    plt.plot(spy_normalized.index, spy_normalized, label=f'SPY ({spy_ret*100:+.1f}%)', color='grey', linewidth=2)
    
    eq_v8.set_index('Date', inplace=True)
    plt.plot(eq_v8.index, eq_v8['Equity'], label=f'V8 PRO ({ret_v8*100:+.1f}%)', color='orange', linewidth=2, linestyle='--')
    
    eq_v9.set_index('Date', inplace=True)
    plt.plot(eq_v9.index, eq_v9['Equity'], label=f'V9 PRO - DXY+TNX+Embargo ({ret_v9*100:+.1f}%)', color='cyan', linewidth=2.5)
    
    plt.title('Backtest : V8 vs V9 PRO (DXY + Taux + Embargo) - 8 Ans', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Capital ($)')
    plt.legend(fontsize=11)
    plt.grid(alpha=0.3)
    plt.savefig('backtest_v9_comparison.png', bbox_inches='tight', dpi=120)
    print("Graphique sauvegarde sous 'backtest_v9_comparison.png'")
